...
namespace Global_FGA_Order_Report
{
    public class ExcelFileAccess : IDisposable
    {
        ...
        public ExcelFileAccess(string filename)
        {
            ...
            {
                MiscUtility.LogHistory(string.Format("Source:{0},  Error:{1}", e.Source, e.Message));
                throw e;
            }
        }
        ...
        ~ExcelFileAccess()
        {
            ...
            {
                MiscUtility.LogHistory("Connection is supposed to be closed by clients instead of waiting for GC.");
            }
